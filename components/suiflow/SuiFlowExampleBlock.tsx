...
const SuiFlowExampleBlock: React.FC<CodeBlockProps> = ({ files = [], defaultTitle, children, darkMode = false }) => {
  const [activeTitle, setActiveTitle] = useState(defaultTitle || (files.length > 0 ? files[0]?.title || '' : ''));
  ...
  return (
    <div className={`${inter.className} bg-opacity-30 ${darkMode ? 'bg-[#0A1428]' : 'bg-[#0A1428]/50'} text-white p-4 rounded relative backdrop-blur-md border border-[#2A3746] max-w-full sm:w-[700px] max-h-96 sm:ml-4`}>
      {files.length > 0 && (
        <div className="flex items-center justify-between mb-2 border-b border-[#2A3746] pb-2">
          <div className="flex space-x-2">
            {files.map(({ title }) => (
              <button
                key={title}
                onClick={() => setActiveTitle(title)}
                className={`flex border-t items-center px-3 py-1 rounded-full text-sm ${title === activeTitle ? 'bg-gradient-to-b from-[#0A1428] via-[#0A2440] to-black text-[#6FBCF0] border-[#3890E3]' : 'bg-[#091428] text-gray-400 border-[#2A3746]'} transition-colors duration-300`}
              >
                <span className="mr-2 text-[#6FBCF0]">{getFileIcon(title)}</span>
                {title}
              </button>
            ))}
          </div>
          <button
            ...
            className="text-gray-400 hover:text-[#6FBCF0] rounded-md transition relative sm:-top-1 sm:-left-2"
          >
            {copied ? (
              <motion.svg
      ...
      <div className="max-w-full overflow-auto max-h-80 mx-auto">
        {children || (
          <SyntaxHighlighter
            ...
            style={CustomTheme} 
            wrapLines
            customStyle={{ padding: '1rem', background: '#0A1428' }}
          >
            {code}
          </SyntaxHighlighter>
        )}
...
export default SuiFlowExampleBlock;
